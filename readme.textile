...
To setup the environment to build run 'ant setup' and then to build a version of bUpload run 'ant main', this will build a version dependent version, and a version independent in the bin folder in the root directory. The version dependent version will not have a minecraft version after the name eg. bUpload-1.0.0.zip is independant bUpload-1.0.0-1.5.2.zip is dependant.
